...
class LoginAPITests: XCTestCase {
    ...
    override func setUpWithError() throws {
        sut = ProviderImpl(session: MockURLSession(), keychainManager: KeychainManager())
    }
    ...
    func test_fetchAuthentication_whenFailed_thenProcessRight() {
        sut = ProviderImpl(session: MockURLSession(makeRequestFail: true), keychainManager: KeychainManager())
        let expectation = XCTestExpectation()
